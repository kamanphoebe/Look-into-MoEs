...
class MoE(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.config = config
        if self.config.single_expert:
            print("using single expert!")
    ...
    def forward(self, x):
        ...
        if hasattr(self.config, 'one_gate') and (self.config.one_gate):
            # print("using one gate!")
            expert_weights = torch.ones_like(expert_weights)
            if hasattr(self.config, 'norm_one_gate') and (self.config.norm_one_gate):
                # print("using normed one gate!")
                expert_weights = expert_weights /  self.num_experts_per_token
...
class MistralModel(MistralPreTrainedModel):
    ...
    def forward(
        ...
        for i, decoder_layer in enumerate(self.layers):
            if decoder_layer_idx is not None and i != decoder_layer_idx:
                continue
...
class MixtralInstructForCausalLM(MistralPreTrainedModel):
